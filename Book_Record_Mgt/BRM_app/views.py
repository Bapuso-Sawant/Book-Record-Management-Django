...
def editBook(request):
    ...
    form = NewBookForm(initial=fields) #initial is used to initialize fields in table
...
def add(request):
    if request.method == 'POST':
        form = NewBookForm(request.POST) #form contain all data
        book = models.Book()
        book.title = form.data['title']
        book.price = form.data['price']
        book.author = form.data['author']
        book.publisher = form.data['publisher']
        book.save()  #commit
    s = "Record Stored Succesfully..."
    return render(request,"BRM_app/record_stored_after_add.html",{'s':s})
